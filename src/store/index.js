...
const fileConstracut = {
  "/": {
    home: {
      "0x6d61": {
        file: ["about.txt", "skill.txt", "icon.png"],
        "about.txt": `
北海道に住んでいる0x6d61です。
...
- Vue
`,
        "icon.png": "https://0x6d61.github.io/icon.png",
        sns: {
          file: ["account.txt"],
          "account.txt": `
...
`;
        ...
        case "ls": {
          const currentPath =
            args.length > 0
              ? deletePathReturn(makePath(args[0], this.state.currentDirectory))
              : this.state.currentDirectory;
          const dirs = currentPath.split("/").filter(d => d !== "");
          const fileName = dirs[dirs.length - 1];
          const newDirs = fileName.includes(".")
          ...
          if (fileList.length > 0 || isFileExistence || isDirExistence) {
            commandExecResult.result =
              args.length > 0 ? args[0] : fileList.join(" ");
          } else {
        ...
        case "cat": {
          if (args.length > 0) {
            const currentPath = deletePathReturn(
              makePath(args[0], this.state.currentDirectory)
            );
            const dirs = currentPath.split("/").filter(d => d !== "");
        ...
        case "cd": {
          if (args[0] === "/") {
            ...
            const { path, error, result } = isChangeDirectory(
              args[0],
              this.state.currentDirectory,
              fileConstracut
            );
